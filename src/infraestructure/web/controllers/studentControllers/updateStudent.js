...
const updateStudent = async (req, res) => {
    try {
        ...
        await db.query('UPDATE student SET name = ?, email = ? WHERE id = ?', [name, email, id]);
        await db.query('DELETE FROM student_matter WHERE student_id = ?', [id]);
        const matterInserts = [];
        for (const matterId of matters) {
            const [matterResult] = await db.query('INSERT INTO student_matter (student_id, matter_id) VALUES (?, ?)', [id, matterId]);
